...
export default function StatCard({ label, value, sub, icon, color = "var(--nx-text)", accent }: StatCardProps) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: "var(--nx-text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>
        {label}
      </div>
      <div style={{ fontSize: 20, fontWeight: 700, color, fontFamily: "monospace", lineHeight: 1.2 }}>
        {value}
      </div>
      {sub && <div style={{ fontSize: 12, color: "var(--nx-text-2)", marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
