...
exports.listBills = asyncHandler(async (req, res) => {
  ...
  const slips = await AdvanceSlip.find(slipFilter).populate('createdBy', 'name').sort({ createdAt: -1 }).lean();
  ...
  const pendingRequests = await BillRequest.find(reqFilter).sort({ createdAt: -1 }).lean();
